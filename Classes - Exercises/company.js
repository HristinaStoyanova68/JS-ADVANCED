class Company {
    ...
    getDepart(name) {
        ...
        if (!depart) {
            const tempDep = { name, employees: [], salaries: [], positions: [] };
            this.departments.push(tempDep);
            return tempDep;
        }
    ...
    bestDepartment() {
        ...
        const combinedEmployyes = best.employees.reduce((a, v, i) => {
            a[i] = [];
            a[i].push(v, best.salaries[i], best.positions[i]);
            return a;
        }, []);

        const sorted = combinedEmployyes.sort((a, b) => {
            return b[1] - a[1] === 0 ? a[0].localeCompare(b[0]) : b[1] - a[1];
        });

        const printData = sorted.map((x) => x.join(' ')).join('\n');
        const averageSalary = (this.getSalariesSum(best) / best.salaries.length).toFixed(2);
